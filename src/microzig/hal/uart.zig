...
fn compute_fbrd(clock: u32, baud: u32) u32 {
    ...
    var fbrd: f32 = (fclock / fdivider) - @as(f32, @floatFromInt(clock / divider));
    fbrd = fbrd * 64 + 0.5;
...
pub fn init(comptime config: Config) void {
    ...
    // Disable
    UART.UART_CR.write_raw(0);
    UART.UART_LCR_H.write_raw(0);
...
pub fn read(context: u32, data: []const u8) ReadError!usize {
    _ = context;
    for (data) |*byte| {
        while (UART.UART_TFR.read().RXFE == 0) {}
        byte.* = UART.UART_DR.read().DATA;
    }
    return data.len;
}
